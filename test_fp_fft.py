...
class TestFFT(unittest.TestCase):
    ...
    def test_butterfly_shift(self):
        ...
        np.testing.assert_equal(out_real, np.array([1, 1, 0, 0]))
        np.testing.assert_equal(out_imag, np.array([0, 0, 0, 0]))
        out_real, out_imag = fp_fft.butterfly_r2(out_real, out_imag, 2, 2, fpt, fpt, fpt, True)
        np.testing.assert_equal(out_real, np.array([1, 0, 0, 0]))
        np.testing.assert_equal(out_imag, np.array([0, 0, 0, 0]))
    ...
    def test_fft_8pt(self):
        d_real = np.array([7, 7, 7, 7, 7, 7, 7, 7])
        d_imag = np.array([0, 0, 0, 0, 0, 0, 0, 0])
        fpt = FixedPointType(8, 7)
        shifts = np.array([1, 1, 1])
        out_real, out_imag = fp_fft.fft_r2(d_real, d_imag, 3, fpt, fpt, fpt, shifts)
        np.testing.assert_equal(out_real, np.array([7,0,0,0,0,0,0,0]))
        np.testing.assert_equal(out_imag, np.array([0,0,0,0,0,0,0,0]))
        shifts = np.array([0, 0, 0])
        out_real, out_imag = fp_fft.fft_r2(d_real, d_imag, 3, fpt, fpt, fpt, shifts)
        np.testing.assert_equal(out_real, np.array([56,0,0,0,0,0,0,0]))
        np.testing.assert_equal(out_imag, np.array([0 ,0,0,0,0,0,0,0]))
